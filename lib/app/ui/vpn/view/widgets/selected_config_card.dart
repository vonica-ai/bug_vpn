...
class SelectedConfigCard extends ConsumerWidget {
  const SelectedConfigCard({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (selectedConfig == null) {
      child = OutlinedButton.icon(
        onPressed: () => ref.read(bottomNavIndexProvider.notifier).update(1),
      ...
      child = Card(
        ...
        child: ListTile(
          onTap: () => ref.read(bottomNavIndexProvider.notifier).update(1),
          leading: CircleAvatar(child: Icon(UIcons.solidRounded.shield_interrogation)),
          title: Text(selectedConfig.name, maxLines: 1, overflow: TextOverflow.ellipsis),
          titleTextStyle: Theme.of(
            context,
          ).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold),
          subtitle: Text('Network : ${selectedConfig.v2rayURL.network.toUpperCase()}'),
